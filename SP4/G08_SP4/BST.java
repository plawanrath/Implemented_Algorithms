...
public class BST<T extends Comparable<? super T>> {
    ...
    BST(T[] arr) {
    	root = convertSortedArrayToBST(arr,0,arr.length-1);
    	size = arr.length;
    }
    
    //Recursive function to create BST from sorted array
    private Entry<T> convertSortedArrayToBST(T[] arr,int start,int end)
    {
    	if(start > end) 
    		return null;
    	int mid = (start + end)/2;
    	Entry<T> node = new Entry<>(arr[mid],null,null,null); //making middle element root
    	node.left = convertSortedArrayToBST(arr, start, mid-1); // construct left subtree
    	node.right = convertSortedArrayToBST(arr,mid+1,end); //construct right subtree
    	return node;
    }
	...
	while(in.hasNext()) {
		...
		System.out.println();
		Comparable[] LOarr = t.levelOrderArray();
		System.out.println("Level Order Array : ");
		for(int i=0; i<t.size; i++) {
		    System.out.print(arr[i] + " ");
		}
		System.out.println();
		...
		BST<Integer> sortedAT = new BST<Integer>(sortedArray);
		System.out.print("BST from Sorted Array :");
		sortedAT.printTree();
		return;
	    }		
	}
